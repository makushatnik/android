...
public class Utils {
    ...
    public static String formatTemperature(Context ctx, double temp) {
        String suffix = "\u00B0";
        if (!isMetric(ctx)) {
            temp = (temp * 1.8) + 32;
        }
        return String.format(ctx.getString(R.string.format_temperature), temp);
    }
    ...
    int getLocationStatus(Context ctx) {
        return PreferenceManager.getDefaultSharedPreferences(ctx).getInt(
                ctx.getString(R.string.pref_location_status_key),
